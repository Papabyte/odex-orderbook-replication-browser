...
_debugChanges(changes)
{
    try
    {
        ...
        console.log(`Method '${method}' will trigger following changes : ${JSON.stringify(changes)}`);
    }
    catch (e)
    {
        return;
...
_queryExchangeState(pair)
{
    ...
    this._connection.callMethod('QueryExchangeState', [pair], function(d, err){
        ...
        if (null !== err)
        {
            if (DEBUG_ENABLED)
            {
                console.log("Could not query exchange for pair '%s' (might be an invalid pair) : err = '%s'", pair, err);
            }
            if (null !== self._logger)
            {
        ...
        self._decodeData.call(self, d, function(data){
            ...
            if (null === data)
            {
                if (DEBUG_ENABLED)
                {
                    console.log("QueryExchangeState returned null for pair '%s' (probably an invalid pair)", pair);
                }
                if (null !== self._logger)
                {
            ...
            let orderBookEvt = {
                ...
                cseq:data.N,
                data:{
                    bids:_.map(data.Z, entry => {
                        return {
                            price:parseFloat(entry.R),
                            size:parseFloat(entry.Q)
                        }
                    }),
                    asks:_.map(data.S, entry => {
                        return {
                            price:parseFloat(entry.R),
                            size:parseFloat(entry.Q)
                        }
            ...
            // emit events
            if (0 != orderBookEvt.data.bids.length || 0 != orderBookEvt.data.asks.length)
            {
...
_processData(data)
{
    ...
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("Exception when trying to process data : %s", e.stack);
        }
    }
}
    ...
    this._decodeData(d, function(data){
        ...
        if (missedCount > 1)
        {
            if (this._resyncOrderBooksAutomatically)
            {
                if (DEBUG_ENABLED)
                {
                    console.log("We missed %d update for market '%s' (full data will be retrieved) : last cseq = %d, current cseq", missedCount, data.M, lastCseq, data.N);
                }
            ...
            if (DEBUG_ENABLED)
            {
                console.log("We missed %d update for market '%s' : last cseq = %d, current cseq", missedCount, data.M, lastCseq, data.N);
            }
        }
        ...
        */
        ...
        // emit events
        if (0 != orderBookUpdateEvt.data.bids.length || 0 != orderBookUpdateEvt.data.asks.length)
        {
...
disconnect()
{
    ...
    if (DEBUG_ENABLED)
    {
        console.log("Connection (bittrex|%d) will be disconnected", this._connectionCounter);
    }
