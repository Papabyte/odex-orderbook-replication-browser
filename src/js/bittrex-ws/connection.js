...
_negotiate(retryCount)
{
             ...
             operation.attempt(function(currentAttempt){
                 ...
                 request(requestOptions, function(error, response, body){
                     ...
                     if (null !== err)
                     {
                         if (DEBUG_ENABLED)
                         {
                             console.log("'negotiate' step error (%d/%s) : %s", attempt, -1 === retryCount ? 'unlimited' : (1 + retryCount), JSON.stringify(err));
                         }
    ...
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("'negotiate' step exception : %s", e.stack);
        }
        return new Promise((resolve, reject) => {
...
_connect(connection, retryCount)
{
        ...
        let retryOptions = {
         ...
         return new Promise((resolve, reject) => {
             operation.attempt(function(currentAttempt){
                 ...
                 attempt = currentAttempt;
								 let doRetry = true;
                 let ws = new WebSocket(queryString);
                 ...
                 ws.onopen = function open(event) {
                     ...
                     if (DEBUG_ENABLED)
                     {
                         console.log("WS connected for connection '%s'", connection.ConnectionId);
                     }
                 ...
                 ws.onmessage = function (message) {
                     ...
                     if (self._logAllWsMessages && DEBUG_ENABLED)
                     {
                         console.log(message);
                     }
                     ...
                     catch (e)
                     {
                        if (DEBUG_ENABLED)
                        {
                            console.log("Received invalid JSON message : %s", message);
                        }
                        return;
                     }
                     // process responses
                     if (undefined !== data.I)
                     {
                         ...
                         else
                         {
                             ...
                             if (DEBUG_ENABLED)
                             {
                                 console.log("Got an error for message %d : err = '%s'", messageId, err);
                             }
                             // do we have a callback for this messageId
                             if (undefined !== self._callbacks[messageId])
                             {
                 ...
                 ws.onerror = function(e) {
                     ...
                     if (DEBUG_ENABLED)
                     {
                         console.log("'connect' step error for connection '%s' (%d/%s) : %s", connection.ConnectionId, attempt, -1 === retryCount ? 'unlimited' : (1 + retryCount), JSON.stringify(err));
                     }
                     skipCloseEvent = true;
                     self._ws = null;
                   //  this.terminate();
              ...
              /*   ws.on('unexpected-response', function(request, response){
                     ...
                     if (DEBUG_ENABLED)
                     {
                         console.log("'connect' step unexpected-response for connection '%s' (%d/%s) : %s", connection.ConnectionId, attempt, -1 === retryCount ? 'unlimited' : (1 + retryCount), JSON.stringify(err));
                     }
                 ...
                 ws.onclose = function(code, reason){
                     ...
                     if (DEBUG_ENABLED)
                     {
                         console.log("WS closed for connection '%s' : code = '%d', reason = '%s'", connection.ConnectionId, code, reason);
                     }
                     self._ws = null;
             ...
             /*    ws.on('ping', function(data){
                     // this is for debugging purpose
                     if (self._logKeepaliveMessages && DEBUG_ENABLED)
                     {
                         console.log(`Got 'ping' message from Bittrex at ${parseInt(Date.now() / 1000.0)}`);
                     }
                     this.pong('', true, true);
                 });
                 ws.on('pong', function(data){
                     ...
                     if (self._logKeepaliveMessages && DEBUG_ENABLED)
                     {
                         console.log(`Got 'pong' message from Bittrex at ${parseInt(Date.now() / 1000.0)}`);
                     }
                     this.isAlive = true;
                 });*/
            });
    ...
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("'connect' step exception : %s", e.stack);
        }
        return new Promise((resolve, reject) => {
            let err = {origin:'client', error:e.message, stack:e.stack};
            reject({attempts:attempt, error:err});
...
_start(connection, retryCount)
{
            ...
            operation.attempt(function(currentAttempt){
                ...
                request(requestOptions, function(error, response, body){
                    ...
                    if (null !== err)
                    {
                        if (DEBUG_ENABLED)
                        {
                            console.log("'start' step error for connection '%s' (%d/%s) : %s", connection.ConnectionId, attempt, -1 === retryCount ? 'unlimited' : (1 + retryCount), JSON.stringify(err));
                        }
                        if (doRetry && operation.retry(err))
                        {
    ...
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("'start' step exception : %s", e.stack);
        }
        return new Promise((resolve, reject) => {
            let err = {origin:'client', error:e.message, stack:e.stack};
            reject({attempts:attempt, error:err});
...
_abort(connection)
{
    try
    {
        ...
        request(requestOptions, function(error, response, body){
            ...
            if (null !== err)
            {
                if (DEBUG_ENABLED)
                {
                    console.log("'abort' step error : %s", JSON.stringify(err));
                }
            }
            else
            {
                if (DEBUG_ENABLED)
                {
                    console.log("'abort' step successfully performed for connection '%s'", connection.ConnectionId);
                }
            }
        });
    }
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("'abort' step exception : %s", e.stack);
        }
    }
}
...
callMethod(method, args, cb)
{
    ...
    catch (e)
    {
        if (DEBUG_ENABLED)
        {
            console.log("Exception when trying to call method %s : %s", method, e.stack);
        }
        return false;
    }
    return true;
...
connect()
{
    ...
    let self = this;
        ...
        self._negotiate(self._retryCount.negotiate).then((result) => {
            ...
            if (DEBUG_ENABLED)
            {
                console.log("'negotiate' step successful after %d attempts : %s", result.attempts, JSON.stringify(self._connection));
            }
						// 'connect' step
						console.log("connect");
            self._connect(self._connection, self._retryCount.connect).then((result) => {
                ...
                if (DEBUG_ENABLED)
                {
                    console.log("'connect' step successful after %d attempts", result.attempts);
                }
                // 'start' step
                self._start(self._connection, self._retryCount.start).then((result) => {
                    ...
                    if (DEBUG_ENABLED)
                    {
                        console.log("'start' step successful after %d attempts", result.attempts);
                    }
                    self.emit('connected', {connectionId:self._connection.ConnectionId});
                }).catch ((e) => {
                    if (DEBUG_ENABLED)
                    {
                        console.log("'start' step stopped after %d attempts : %s", e.attempts, JSON.stringify(e.error));
                    }
                ...
                if (DEBUG_ENABLED)
                {
                    console.log("'connect' step stopped after %d attempts : %s", e.attempts, JSON.stringify(e.error));
                }
            ...
            if (DEBUG_ENABLED)
            {
                console.log("'negotiate' step stopped after %d attempts : %s", e.attempts, JSON.stringify(e.error));
            }
